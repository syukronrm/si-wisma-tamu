<?php include("template/header.php");?>

  <body style="background-image: url(back.jpg); background-attachment:fixed; " >
    <div class="container">
      <div class="masthead">
        <h1 style="text-align:right;color:white">
           <i><b>WISMA TAMU</b></i>
        </h1>
        <h3 style="text-align:right;color:white"> 
            <i><b>Institut Teknologi Sepuluh Nopember</b></i>
        </h3>
          <ul class="nav nav-tabs" >
              <li class="active">
                  <a href="#"> Home</a>
              </li>
              <li>
                <a href="#" data-toggle="dropdown" class="dropdown-toggle" style="color:white">Reservasi <b class="caret"></b></a>
                <ul class="dropdown-menu">
                  <li>
                    <a href="flamboyan.php">Famboyan</a>
                  </li>
                  <li>
                    <a href="#">Bougenville</a>
                  </li>
                  <li>
                    <a href="#">Jasmine</a>
                  </li>
                   <li>
                    <a href="#">Tambah Wisma</a>
                  </li>
                </ul>
              </li>
          </ul>
	<!--	    <ul class="nav nav-justified">
          <li class="active">
            <a href="home.html">Home</a>
          </li>
          <li >
                  <a href="#">Reservasi</a>            

          </li>
        </ul>
      <!-- Example row       </div>
      <!-- Jumbotron -->

      <div class="row" style="padding-top:20px;">
      <div class="col-md-12" >
        <div class="panel panel-default" style="background-color:#eaeae1">
          <div class="panel-body">
            <center> <h3><b>SELAMAT DATANG <br> SISTEM INFORMASI WISMA TAMU </b></h3>  <center> 
            <center><p> <img src="home.jpg" alt="Kepegawaian" style="height:300px"></center>
            <br><br>
          </div>
        </div>
      </div>
      <!-- Site footer -->
      <div class="footer" style="color:white">
        <center><p>&copy; TC 14</p></center>
      </div>
    </div>
    <!-- /container -->
  </body>

</html>